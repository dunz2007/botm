    ...
    try {
        ...
        switch (logMessageType) {
            ...
            case "log:user-nickname": {
                ...
                if (global.configModule[this.config.name].sendNoti) api.sendMessage(`[ CẬP NHẬT NHÓM ]\nĐã cập nhật biệt danh của người dùng ${logMessageData.participant_id}\nThành: ${(logMessageData.nickname.length == 0) ? "tên gốc": logMessageData.nickname}`, threadID, async (error, info) => {
                    if (global.configModule[this.config.name].autoUnsend) {
                        await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name].timeToUnsend * 1000));
                        return api.unsendMessage(info.messageID);
                    } else return;
