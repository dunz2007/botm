...
if (!ADMINBOT.includes(senderID) && adminbot.adminOnly == true) return api.sendMessage('[ MODE ] - Chỉ admin bot mới có thể sử dụng bot\n\n📌 Đang fix', threadID, messageID)
if (!NDH.includes(senderID) && !ADMINBOT.includes(senderID) && adminbot.ndhOnly == true) return api.sendMessage('[ MODE ] - Chỉ người hỗ trợ bot mới có thể sử dụng bot', threadID, messageID)
    ...
    if (dataAdbox.adminbox.hasOwnProperty(threadID) && dataAdbox.adminbox[threadID] == true && !ADMINBOT.includes(senderID) && !findd && event.isGroup == true) return api.sendMessage('[ MODE ] - Chỉ quản trị viên nhóm mới có thể sử dụng bot', event.threadID, event.messageID)
...
if (!ADMINBOT.includes(senderID) && data_disable_cmd_category[threadID][cmd_category] == true)return api.sendMessage(`❎ Nhóm bạn không được phép sử dụng nhóm lệnh ${cmd_category}`, threadID);
    ...
    if (command.config.hasPermssion > permssion) return api.sendMessage(global.getText("handleCommand", "permssionNotEnough", command.config.name), event.threadID, event.messageID);
      ...
      return api.sendMessage(`📌 Bạn đang sử dụng lệnh quá nhanh\n Vui lòng thử lại sau ${((timestamps.get(senderID) + expirationTime - dateNow)/1000).toString().slice(0, 5)}s`, threadID, messageID);
