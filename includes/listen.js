   ...
   var data_anti = JSON.parse(fs.readFileSync(global.anti, "utf8"));
    ...
    if (logMessageType === "log:unsubscribe") {
     ...
     let form_mm_dd_yyyy = (input = '', split = input.split('/'))=>`${split[1]}/${split[0]}/${split[2]}`;
	  ...
	  if (!find_thuebot)return send(`Nhóm của bạn chưa thuê bot, vui lòng liên hệ Admin để thuê bot.\n\nFacebook: fb.com/niiozic.dev\nZalo: 0368269220\nTelegram: t.me/niiozic`);
	  if (new Date(form_mm_dd_yyyy(find_thuebot.time_end)).getTime() <= Date.now()+25200000)return send(`Nhóm của bạn đã hết hạn thuê bot vui lòng thanh toán để tiếp tục gia hạn, liên hệ Admin.\n\nFacebook: fb.com/niiozic.dev\nZalo: 0368269220\nTelegram: t.me/niiozic`);
	  };
